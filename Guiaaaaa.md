...
## 19. Taller E-commerce (MySQL Avanzado) — Eventos programados

> **Nota:** este taller usa **MySQL**, no PostgreSQL como las fases anteriores. La sintaxis de eventos programados (`CREATE EVENT`) es propia de MySQL/MariaDB y no existe igual en PostgreSQL. Además, solo se pudo leer parcialmente el enunciado del primer ejercicio en la foto, así que abajo se aclara qué se asumió del esquema.

### 19.1 Teoría: Event Scheduler de MySQL

MySQL tiene un planificador de tareas interno (el **Event Scheduler**) que ejecuta bloques de SQL de forma automática, en una fecha específica o de manera recurrente — es el equivalente a un `cron` pero dentro de la base de datos.

**Activar el planificador** (una sola vez, a nivel de servidor):
```sql
...
**Sintaxis general de un evento:**
```sql
CREATE EVENT nombre_evento
ON SCHEDULE
    EVERY 1 DAY               -- o: AT '2026-01-01 00:00:00'
STARTS CURRENT_TIMESTAMP
DO
    -- bloque de SQL a ejecutar
    UPDATE tabla SET columna = valor WHERE condicion;
```

- `ON SCHEDULE AT ...`: se ejecuta **una sola vez** en la fecha/hora indicada.
- `ON SCHEDULE EVERY ... [STARTS ...] [ENDS ...]`: se ejecuta de forma **recurrente** (cada hora, día, semana, etc.).
- `ON COMPLETION [NOT] PRESERVE`: si el evento es de una sola vez, define si se borra solo después de ejecutarse (`NOT PRESERVE`, por defecto) o si se mantiene visible en el catálogo (`PRESERVE`).

**Administrar eventos existentes:**
```sql
SHOW EVENTS;                          -- listar eventos
ALTER EVENT nombre_evento DISABLE;    -- desactivar sin borrar
ALTER EVENT nombre_evento ENABLE;     -- reactivar
DROP EVENT nombre_evento;             -- eliminar
```

### 19.2 Ejercicio 1 — Desactivación automática de productos sin rotación

**Enunciado (reconstruido de la foto):** para mantener el catálogo optimizado y evitar ofrecer artículos que ya no rotan, la plataforma necesita descontinuar automáticamente aquellos productos que no han tenido ventas en el último año (365 días), liberando espacio operativo. Esto se debe resolver con un evento programado.

**Supuesto de esquema** (no visible completo en la foto — ajusta los nombres reales de tus tablas/columnas):
- `productos(id, nombre, activo, ...)` — `activo` es un booleano/tinyint que indica si el producto sigue a la venta.
- `ventas(id, fecha, ...)`
- `detalle_ventas(id, venta_id, producto_id, ...)` — relaciona cada venta con sus productos.

```sql
-- Paso 1: asegurarse de que el planificador esté activo
SET GLOBAL event_scheduler = ON;

-- Paso 2: crear el evento programado
DELIMITER $$

CREATE EVENT desactivar_productos_sin_rotacion
    ...
    WHERE p.activo = 1
      AND NOT EXISTS (
          ...
          WHERE dv.producto_id = p.id
            AND v.fecha >= CURDATE() - INTERVAL 365 DAY
      );
END$$
...
- El evento corre **una vez al día** (`EVERY 1 DAY`) y revisa todos los productos activos.
- Para cada producto, verifica con `NOT EXISTS` si **no** hubo ninguna venta asociada en los últimos 365 días.
- Si no hubo ventas recientes, lo marca como inactivo (`activo = 0`) en lugar de borrarlo, preservando el historial.
- `DELIMITER $$` es necesario porque el cuerpo del evento tiene punto y coma internos; sin cambiar el delimitador, MySQL cortaría la sentencia antes de tiempo.

**Verificación:**
```sql
SHOW EVENTS LIKE 'desactivar_productos_sin_rotacion';
...
> Cuando tengas el documento completo (o mejores fotos), se puede ajustar esta sección con los nombres reales de las tablas del taller y sumar los demás ejercicios que falten.

---

## 20. Chuleta rápida final
